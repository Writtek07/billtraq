...
#Check in prod -> [Aarav Gupta,Khushi Yadav]
present = []
no_inv = []
no_stud = []
first_name.each_with_index do |fn, id|
  if last_name[id].present?
    stu = Student.find_by(first_name: fn, last_name: last_name[id],grade: grade[id])
  else
    stu = Student.find_by(first_name: fn, grade: grade[id])
  end
  if stu.present?
    if stu.invoices.present?
      present << stu.id
    else
      no_inv << stu.id
    end
...
Student.where(first_name: first_name[5], last_name: last_name[5],grade: grade[5]).first