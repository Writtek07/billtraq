namespace :twilio do
    desc 'Send daily invoice SMS to admin'
    task send_invoice_sms: :environment do
        # phone_number = ['+918839390992','+919827157664','+919329385300']
        phone_number = ['+918839390992']
        today = Date.today.strftime("%d-%m-%Y")
        cash_total = Invoice.where(created_at: Time.zone.today.beginning_of_day .. Time.zone.now, payment_mode: 'Cash').kept.sum(:total)
        online_total = Invoice.where(created_at: Time.zone.today.beginning_of_day ..Time.zone.now, payment_mode: 'Online').kept.sum(:total)
        cheque_total = Invoice.where(created_at: Time.zone.today.beginning_of_day ..Time.zone.now, payment_mode: 'Cheque').kept.sum(:total)
        ...
        # message = "Total fees for #{today} - #{total}. Cash - #{cash_total} | Online - #{online_total} | Cheque - #{cheque_total}. #{backdated_inv_count} backdated invoices entered today with total amount - #{backdated_inv_sum} with these stats - \n #{backdated_notes_invoices}."
        message = "Total fees for #{today} - #{total}. Cash - #{cash_total} | Online - #{online_total} | Cheque - #{cheque_total}."
