namespace :twilio do
    ...
    task send_invoice_sms: :environment do
        ...
        backdated_inv = Invoice.where(created_at: Time.zone.today.beginning_of_day ..Time.zone.today.end_of_day).where.not(notes: "").kept.pluck(:notes).uniq        
        backdated_notes_invoices = Hash.new(0)
        
        Invoice.where(notes: backdated_inv).each do |inv|             
